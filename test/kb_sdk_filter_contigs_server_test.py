...
class kb_sdk_filter_contigsTest(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        token = environ.get('KB_AUTH_TOKEN', None)
        cls.ctx = {'token': token, 'provenance': [{'service': 'kb_sdk_filter_contigs',
        ...
        for nameval in config.items('kb_sdk_filter_contigs'):
            cls.cfg[nameval[0]] = nameval[1]
        cls.wsURL = cls.cfg['workspace-url']
        cls.wsClient = workspaceService(cls.wsURL, token=token)
        cls.serviceImpl = kb_sdk_filter_contigs(cls.cfg)
    ...
    def getWsName(self):
        ...
        wsName = "test_kb_sdk_filter_contigs_" + str(suffix)
